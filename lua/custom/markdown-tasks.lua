-- Markdown task toggling (mark a task done / undone).
-- Adapted from https://github.com/linkarzu/dotfiles-latest
--   (neobean/lua/config/modules/markdown_tasks.lua, commit 8d961fc)
--
-- Behavior of M.toggle_done():
--   * On a `- [ ]` bullet (or anywhere inside its wrapped text), mark it `- [x]`,
--     stamp a `` `done: <timestamp>` `` label after the checkbox, and move the
--     whole item under a "## Completed Tasks" heading (created if missing).
--   * On an item already labeled `` `untoggled` ``, mark it done in place.
--   * On an item labeled `` `done: ...` ``, revert it to `` `untoggled` `` and uncheck it.
--
-- The buffer-local <M-x> (Alt+x) keymap is registered for markdown files at the
-- bottom of this file via a FileType autocmd.
...
-- Register a buffer-local Alt+x (<M-x>) keymap for markdown files so the binding
-- only applies where it makes sense and stays out of the way elsewhere.
vim.api.nvim_create_autocmd('FileType', {
  pattern = { 'markdown' },
  callback = function(event)
    vim.keymap.set('n', '<M-x>', function()
      M.toggle_done()
    end, { buffer = event.buf, desc = 'Markdown: toggle task done/undone' })
  end,
})
